...
VMWCHAR wtest_text[100];
VMINT xBaseline;
VMINT hZZZ;
VMCHAR debug_text[100];
...
void vm_main(void) {
   ...
   vm_ascii_to_ucs2(wtest_text, (strlen("WWp") + 1) * 2, "WWp"); //daryti fiksuota skaiciu

   xBaseline = vm_graphic_get_string_baseline(wtest_text);
   hZZZ = vm_graphic_get_string_height(wtest_text);
...
void handle_sysevt(VMINT message, VMINT param) {
    ...
    switch (message) {
        ...
        case VM_MSG_PAINT:
            ...
            if (flightMode == VM_TRUE) { display_text_line(buffer, "Please turn Flight mode off !", 0, filledDsplByLines, nscreen_width, nscreen_height, 1, 1, VM_COLOR_BLACK, VM_FALSE);}
            ...
            if(second == VM_TRUE){
              ...
              display_text_line(buffer, CellNameStatusZ, 0, filledDsplByLines, nscreen_width, nscreen_height, 1, 1, VM_COLOR_BLACK, VM_FALSE);
            } else {
...
void handle_keyevt(VMINT event, VMINT keycode) {
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_LEFT_SOFTKEY) { //VM_KEY_NUM1 VM_KEY_LEFT_SOFTKEY
        if (flightMode == VM_FALSE && LocFound == 1) {
           //second = VM_TRUE;
           //vm_set_app_desired_input_mode(VM_INPUT_MODE_SMART_FIRST_UPPERCASE_LITHUANIAN);
           //vm_input_text3(NULL, 100, 32, save_text);
           vm_input_text3(CellNameStatus, 256 , 32, save_text);
        }
    ...
    //VMINT baseline = vm_graphic_get_string_baseline(refStr);
        ...
        //sprintf(debug_text, "hZZZ = %d, xBaseline = %d, full_height = %d, baseline = %d", hZZZ, xBaseline, full_height, baseline);
        //display_text_line(buffer, debug_text, 0, filledDsplByLines, nscreen_width, nscreen_height, 0, 1, VM_COLOR_BLACK, VM_FALSE);
        //display_text_line(buffer, "HHHHHHHEEEEEEE", 0, filledDsplByLines, nscreen_width, nscreen_height, 1, 1, VM_COLOR_BLACK, VM_FALSE);
    }
...
void display_text_line(VMUINT8 *disp_buf, VMSTR str, VMINT x, VMINT y, VMINT width, VMINT height, VMINT betlines, VMINT startLine, VMINT color, VMBOOL fix_pos) {
    ...
    if (y == 0) {
        fill_white();
        //nheight = top_margin;
        //filledDsplByLines = top_margin;

    }

//    if (y == 0) {
//        fill_white();
//        nheight = top_margin + xBaseline;
//    }
    ...
    VMINT nline_height = full_height - (descender / 2) + 1 + betlines;  //kazkiek gerai ++ // = 16
    ...
    if (0 != vm_chset_convert(VM_CHSET_UTF8, VM_CHSET_UCS2, str, (VMSTR)ucstr, 4 * (strlen(str) + 1))) {
        vm_free(ucstr);
        return;
    }
    ...
    while (!is_end) {
        ...
        //if (nheight + nline_height + bottom_margin + top_margin > height) { //------------------------------------------------------top_margin ?
        if (nheight + nline_height + bottom_margin > height) {
            ...
            vm_graphic_fill_rect(disp_buf, 0, height - lineHeight - bottom_margin, nscreen_width, lineHeight + bottom_margin, VM_COLOR_WHITE, VM_COLOR_WHITE);

            /* nustatome naują aukštį su viršutine parašte */
            //nheight = height - lineHeight - bottom_margin + top_margin; //??????????????????????????????????????? +top_margin
            nheight = height - lineHeight - bottom_margin; // turi buti geras
        ...
        if (nlines >= startLine) {
            ...
            vm_graphic_textout_by_baseline(disp_buf, x, nheight, ucshead, (ucstail - ucshead), (VMUINT16)color, xBaseline);
...
void timer(int a){
	...
	//vm_cell_info_struct** bs = vm_cell_get_nbr_cell_info();  //gauti kaimyniniu stociu informacija
        ...
        sprintf(t, "%02d.%02d.%02d %02d:%02d:%02d", curr_time.year % 100, curr_time.mon, curr_time.day, curr_time.hour, curr_time.min, curr_time.sec);
        display_text_line(buffer, t, 0, filledDsplByLines, nscreen_width, nscreen_height, 1, 1, VM_COLOR_BLACK, VM_FALSE); //---------------------------
        //display_text_line(buffer, t, 0, filledDsplByLines, nscreen_width, nscreen_height, 0, 1, VM_COLOR_BLACK, VM_FALSE); //---------------------------
        ...
        //vm_chset_convert(VM_CHSET_UCS2, VM_CHSET_UTF8, (VMSTR)CellNameStatus, CellNameStatusZ, 2 * (vm_wstrlen(CellNameStatus) + 1));
        vm_chset_convert(VM_CHSET_UCS2, VM_CHSET_UTF8, (VMSTR)CellNameStatus, CellNameStatusZ, sizeof(CellNameStatusZ));

        display_text_line(buffer, CellNameStatusZ, 0, filledDsplByLines, nscreen_width, nscreen_height, 1, 1, VM_COLOR_BLACK, VM_FALSE); //-------
        //display_text_line(buffer, CellNameStatusZ, 0, filledDsplByLines, nscreen_width, nscreen_height, 0, 1, VM_COLOR_BLACK, VM_FALSE); //-------
	...
	//	sprintf(s, "cellid:%d, lac:%d, signal:%d", bs[i]->ci, bs[i]->lac, bs[i]->rxlev-110);
        //        display_text_line(buffer, s, 0, filledDsplByLines, nscreen_width, nscreen_height, 2, 1, VM_COLOR_BLACK, VM_FALSE);
	//}
...
int CheckCurrentCell() {
    ...
    vm_chset_convert(VM_CHSET_UTF8, VM_CHSET_UCS2, cellNameAscii, (VMSTR)CellName, 4 * (strlen(cellNameAscii) + 1));
    ...
    if (db_buf) {
        ...
        if (param) {
            ...
            if (start) {
                ...
                vm_chset_convert(VM_CHSET_UTF8, VM_CHSET_UCS2, start, (VMSTR)CellName, 4 * (strlen(start) + 1));
                if (!showCellID)
                    vm_wstrcpy(CellNameStatus, CellName);
            }
            LocFound = 1;
    ...
    if (!LocFound && AutoLocation == VM_TRUE) {
        if (vm_wstrlen(SIwstr) == 0) {
            //vm_get_time(&curr_time);
            //sprintf(t, "Unknown %02d%02d%02d%02d%02d%02d", curr_time.year % 100, curr_time.mon, curr_time.day, curr_time.hour, curr_time.min, curr_time.sec);
            //vm_ascii_to_ucs2(SIwstr, sizeof(SIwstr), t);
            vm_ascii_to_ucs2(SIwstr, sizeof(SIwstr), "Unknown");
            //vm_chset_convert(VM_CHSET_UTF8, VM_CHSET_UCS2, "Unknown", (VMSTR)SIwstr, 4 * (strlen("Unknown") + 1));
        }
...
int AddTo_db(VMWSTR wstr) {
    ...
    vm_chset_convert(VM_CHSET_UCS2, VM_CHSET_UTF8, (VMSTR)wstr, asciiValue, sizeof(asciiValue));
...
void UpdateCellName(char *newName) {
    ...
    vm_chset_convert(VM_CHSET_UTF8, VM_CHSET_UCS2, newName, (VMSTR)wname, 4 * (strlen(newName) + 1));
    AddTo_db(wname);
}
 ...
 VMINT length;
    ...
    if (state == VM_INPUT_OK) {
        ...
        if (length > 0) {
           ...
           vm_chset_convert(VM_CHSET_UCS2, VM_CHSET_UTF8, (VMSTR)text, t, sizeof(t));
